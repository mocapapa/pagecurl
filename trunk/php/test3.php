...
//for ($w = $K0; $w >= -$E - $step; $w -= $step) {
  ...
  for ($y = -H/2; $y < H/2; $y++) {
	...
	if ((-W/2 <= $x4) && ($x4 < W/2) && (-H/2 <= $y4) && ($y4 < H/2)) {
	  ...
	  if ($m_mode==2) {
	    // 裏紙
	    $r41 = $g41 = $b41 = URA;
	    if ((int)($ix4+1) > W-1) $r42 = $g42 = $b42 = 0;
	    else $r42 = $g42 = $b42 = URA;
	    if ((int)($iy4+1) > H-1) $r43 = $g43 = $b43 = 0;
	    else $r43 = $g43 = $b43 = URA;
	    $r = $r41 + ($r42 - $r41)*$dx + ($r43 - $r41)*$dy;
	    $g = $g41 + ($g42 - $g41)*$dx + ($g43 - $g41)*$dy;
	    $b = $b41 + ($b42 - $b41)*$dx + ($b43 - $b41)*$dy;
	    if ($r < 0) $r = 0;
	    if ($g < 0) $g = 0;
	    if ($b < 0) $b = 0;
	    if ($r > 255) $r = 255;
	    ...
	    if ((int)($ix4+1) > W-1) $color2 = 0;
	    else $color2= imagecolorat($img1_in, $ix4+1, $iy4);
	    if ((int)($iy4+1) > H-1) $color3 = 0;
	    else $color3= imagecolorat($img1_in, $ix4, $iy4+1);
	...
	if ((-W/2 <= $x3) && ($x3 <= W/2) && (-H/2 <= $y3) && ($y3 <= H/2)) {
	  ...
	  if ($m_mode==2) {
	    // 裏紙
	    $r31 = $g31 = $b31 = URA;
	    if ((int)($ix3+1) > W-1) $r32 = $g32 = $b32 = 0;
	    else $r32 = $g32 = $b32 = URA;
	    if ((int)($iy3+1) > H-1) $r33 = $g33 = $b33 = 0;
	    else $r33 = $g33 = $b33 = URA;
	    $r = $r31 + ($r32 - $r31)*$dx + ($r33 - $r31)*$dy;
	    $g = $g31 + ($g32 - $g31)*$dx + ($g33 - $g31)*$dy;
	    $b = $b31 + ($b32 - $b31)*$dx + ($b33 - $b31)*$dy;
	    if ($r < 0) $r = 0;
	    if ($g < 0) $g = 0;
	    if ($b < 0) $b = 0;
	    if ($r > 255) $r = 255;
	    ...
	    if ((int)($ix3+1) > W-1) $color2 = 0;
	    else $color2= imagecolorat($img1_in, $ix3+1, $iy3);
	    if ((int)($iy3+1) > H-1) $color3 = 0;
	    else $color3= imagecolorat($img1_in, $ix3, $iy3+1);
	    ...
	    // 裏画像
	    $color1 = imagecolorat($img1_in, $ix3, $iy3);
	    if ($ix3+1 > W-1) $color2 = 0;
	    else $color2= imagecolorat($img1_in, $ix3+1, $iy3);
	    if ($iy3+1 > H-1) $color3 = 0;
	    else $color3= imagecolorat($img1_in, $ix3, $iy3+1);
	    $r31 = ($color1 & 0xff0000)>>16;
	    $g31 = ($color1 & 0xff00)>>8;
	    $b31 = ($color1 & 0xff);
	  ...
	  $color1 = imagecolorat($img1_in, $ix2, $iy2);
	  if ($ix2+1 > W-1) $color2 = 0;
	  else $color2= imagecolorat($img1_in, $ix2+1, $iy2);
	  if ($iy2+1 > H-1) $color3 = 0;
	  else $color3= imagecolorat($img1_in, $ix2, $iy2+1);
	  $r21 = ($color1 & 0xff0000)>>16;
	  $g21 = ($color1 & 0xff00)>>8;
	  $b21 = ($color1 & 0xff);
	  $r22 = ($color2 & 0xff0000)>>16;
  ...
  $filename = sprintf("NewImage-test3-%d-%d-%03d.png", $m_mode, $c_mode, $count++);
